...
<body>
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="text-center">
              <h2 class="mb-4 text-primary font-weight-bold">Wyjątkowe Realizacje Stanu Surowego</h2>
              <p class="mb-5">Otwórz drzwi do nieograniczonego potencjału! Specjalizujemy się w tworzeniu solidnych konstrukcji, gdzie pomysł staje się rzeczywistością. Realizujemy budowę fundamentów, ścian, dachów oraz instalację elektryczną i sanitarną. Oferujemy profesjonalizm, który pozwoli Ci zbudować solidne podstawy dla Twoich marzeń.</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div  class="container">
        <div class="d-flex justify-content-between align-items-center">
            <h3 class="mb-0">Stan Surowy</h3>
            @if (Auth::check() && Auth::user()->isAdmin())
            <a href="{{ route('basicState.create') }}" class="btn btn-success">Dodaj</a>
            @endif
        </div>
        <div class="row">
            ...
            <div class="col-md-4">
                <div class="card" >
                    @if ($bs->name === 'Ściany wewnętrzne i działowe')
                    <div class="hot-badge">HOT!!!</div>
                    @endif
                    <img src="{{ asset($bs->image_path) }}" alt="{{ $bs->name }} Image"  class="card-img-top" alt="Fundamenty">
                    <div class="card-body">
                        ...
                        <div class="d-grid gap-2 d-md-block">
                            ...
                            @endif
                            <button class="btn btn-secondary" type="button" onclick="showServiceModal('{{ $bs->name }}', '{{ $bs->longdesc }}')">Czytaj dalej</button>
                            <div class="d-grid gap-2 d-md-block">
                            @if (Auth::check() && Auth::user()->isAdmin())
                            <a href="{{ route('basicState.edit', $bs->id) }}" class="btn btn-warning">Edytuj</a>
                            <form method="POST" action="{{ route('basicState.destroy', $bs->id) }}" style="display: inline-block">
                                @csrf
                                @method('DELETE')
    ...
    <div id="service-modal" class="modal fade" tabindex="-1" role="dialog">
      ...
      {{-- <!-- Modal dla edycji usługi -->
<div id="edit-service-modal" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Edycja usługi</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{ route('clients.update', $bs->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <!-- Dodaj pola do edycji danych usługi -->
                    <div class="form-group">
                        <label for="service-name">Nazwa usługi:</label>
                        <input type="text" class="form-control" id="service-name" name="name" value="{{ $bs->name }}">
                    </div>
                    <div class="form-group">
                        <label for="service-description">Opis usługi:</label>
                        <textarea class="form-control" id="service-description" name="description">{{ $bs->description }}</textarea>
                    </div>
...
<script>
    // Funkcja wywoływana po kliknięciu przycisku "Edytuj"
    function showEditModal(serviceId) {
        // Pobierz dane usługi z serwera i ustaw wartości pól formularza modalu
        // Możesz użyć JavaScript/jQuery lub AJAX, aby pobrać dane z serwera i wypełnić pola formularza
        // Przykład użycia jQuery.ajax():
        $.ajax({
            url: '/services/' + serviceId + '/edit',
            method: 'GET',
            success: function(response) {
                $('#service-name').val(response.name);
                $('#service-description').val(response.description);
                // Ustaw inne pola formularza
                // ...
                // Wyświetl modal
